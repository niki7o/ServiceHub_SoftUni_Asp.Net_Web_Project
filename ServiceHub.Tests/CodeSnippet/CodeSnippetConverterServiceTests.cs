...
namespace ServiceHub.Tests.CodeSnippet
{
    public class CodeSnippetConverterServiceTests
    {
        ...
        public async Task ConvertCodeAsync_ShouldReturnEmptyCodeAndMessage_WhenSourceCodeIsEmpty()
        {
            ...
            var response = await _service.ConvertCodeAsync(request, false);
        ...
        public async Task ConvertCodeAsync_ShouldReturnUnsupportedMessage_ForUnknownTargetLanguage()
        {
            ...
            _mockLogger.Verify(
                x => x.Log(
                    ...
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((o, t) => o.ToString().Contains($"Attempting to convert code from {request.SourceLanguage} to {request.TargetLanguage}. IsBusinessUser: False")),
                    It.IsAny<Exception>(),
                    (Func<It.IsAnyType, Exception, string>)It.IsAny<object>()),
                Times.Once
            );
        ...
        [Theory]
        [InlineData("javascript", "c#")]
        [InlineData("c#", "javascript")]
        [InlineData("php", "python")]
        [InlineData("python", "php")]
        public async Task ConvertCodeAsync_ShouldForbidLockedLanguagesForNonBusinessUser(string sourceLang, string targetLang)
        {
            var request = new CodeSnippetConvertRequestModel
            {
                SourceCode = "Some code;",
            ...
            Assert.Contains("Достъпът до JavaScript и PHP конвертиране е само за Бизнес Потребители.", response.ConvertedCode);
            Assert.Contains("Достъпът до JavaScript и PHP конвертиране е само за Бизнес Потребители. Моля, надстройте акаунта си.", response.Message);
            _mockLogger.Verify(
                x => x.Log(
                    LogLevel.Warning,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((o, t) => o.ToString().Contains($"Unauthorized access attempt for locked language by non-business user. Source: {sourceLang}, Target: {targetLang}")),
                    It.IsAny<Exception>(),
                    (Func<It.IsAnyType, Exception, string>)It.IsAny<object>()),
                Times.Once
        ...
        [InlineData("", "")]
        [InlineData(null, null)]
        public void ConvertToCamelCase_ShouldConvertCorrectly(string input, string expected)
        {
            var result = _service.GetType().GetMethod("ConvertToCamelCase", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                                 .Invoke(_service, new object[] { input });
            Assert.Equal(expected, result);
        }

        [Theory]
        [InlineData("MyMethod", "my_method")]
        [InlineData("SomeVariable", "some_variable")]
        [InlineData("AnotherOne", "another_one")]
        [InlineData("", "")]
        [InlineData(null, null)]
        public void ConvertToSnakeCase_ShouldConvertCorrectly(string input, string expected)
        {
            var result = _service.GetType().GetMethod("ConvertToSnakeCase", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                                 .Invoke(_service, new object[] { input });
            Assert.Equal(expected, result);
        }
    }
